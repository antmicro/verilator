...
def _get_verilator_info():
    ...
    verilator_bin = os.environ.get('VERILATOR_CNT_RND', 'verilator')
    version_line = ''
    try:
        result = subprocess.run(
            [verilator_bin, '--version'], capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
...
def generate_report(module_name, solutions, iterations, observed, jsd_nats, coverage, distinct_hit, freq_variance, compile_time, sim_time, input_path, method='verilator', unsat_pct=0.0, uniqueness_pct=100.0, freq_svg_name=None, cov_svg_name=None):
...
def write_summary_table():
    ...
    with open(rst_path, 'a') as f:
        ...
        f.write('   :widths: 24 8 10 10 12 10 10 40\n\n')
        f.write('   * - Benchmark\n')
        f.write('     - Solutions\n')
        f.write('     - Coverage\n')
        f.write('     - JSD (x100)\n')
...
def run_experiment(module_name, solutions, iterations=None, method=None):
    ...
    generate_report(module_name, solutions, ITERATIONS, observed, jsd_nats, coverage, distinct_hit, freq_variance, compile_time, sim_time, INPUT_PATH, method=method, unsat_pct=unsat_pct, uniqueness_pct=uniqueness_pct, freq_svg_name=freq_svg_name, cov_svg_name=cov_svg_name)
