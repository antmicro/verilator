...
int sc_main(int argc, char* argv[]) {
    ...
    tfp->spTrace()->addTraceVar(makeScSignalExternalVariable(bit_signal_to_trace, *tfp));
    tfp->spTrace()->addTraceVar(makeScSignalExternalVariable(bit_signal_to_trace2, *tfp));
    tfp->spTrace()->addTraceVar(makeScSignalExternalVariable(int_signal_to_trace, *tfp));
    tfp->spTrace()->addTraceVar(makeScSignalExternalVariable(int_signal_to_trace2, *tfp));
    ...
    while (!Verilated::gotFinish()) {
        ...
        std::printf(
            ...
            sc_core::sc_time_stamp().to_double(), bit_signal_to_trace.read(),
            int_signal_to_trace.read(), bit_signal_to_trace2.read());
