    ...
    for (std::string::const_iterator pos = format.cbegin(); pos != format.cend(); ++pos) {
        if (!inPct && pos[0] == '%') {
            ...
            if (formatAttr == VL_VFORMATATTR_COMPLEX) {  // printed as string
                ...
                if (formatAttr == VL_VFORMATATTR_SIGNED_FOURSTATE
                    ...
                    if (lbits <= VL_QUADSIZE) {
                        ...
                        strwidexz.resize(2);
                        strwidexz[0] = static_cast<IData>(ldxz);
                        strwidexz[1] = static_cast<IData>(ldxz >> VL_EDATASIZE);
                        lwxzp = WDataInP::external(strwidexz.data());
                    } else {
                        lwxzp = WDataInP::external(va_arg(ap, EData*));
                        ldxz = lwxzp[0];
                    }
                }
                if (fmt == 'p') {
            ...
            case 'd': {  // Signed/unsigned decimal
                ...
                buffer.resize(words);
                const WDataInP zerosp = WDataInP::external(zeros.data());
                const WDataOutP bufferp = WDataOutP::external(buffer.data());
                if (formatAttr == VL_VFORMATATTR_SIGNED || formatAttr == VL_VFORMATATTR_UNSIGNED
                    || VL_EQ_W(words, zerosp, lwxzp)) {
                    if (formatAttr == VL_VFORMATATTR_SIGNED
                        ...
                        if (lbits <= VL_QUADSIZE) {
                            ...
                            if (VL_SIGN_E(lbits, lwp[VL_WORDS_I(lbits) - 1])) {
                                ...
                                VL_NEGATE_W(VL_WORDS_I(lbits), WDataOutP::external(neg.data()),
                                            lwp);
                                append
                                    = "-"s + VL_DECIMAL_NW(lbits, WDataInP::external(neg.data()));
                            } else {
                ...
                } else if (VL_EQ_W(words, zerosp,
                                   VL_CLEAN_WW(lbits, bufferp, VL_NOT_W(words, bufferp, lwxzp)))) {
                    if (VL_EQ_W(words, zerosp, lwp)) {
                        append = "z";
                    } else if (VL_EQ_W(
                                   words, zerosp,
                                   VL_CLEAN_WW(lbits, bufferp, VL_NOT_W(words, bufferp, lwp)))) {
                        append = "x";
                    } else {
                        append = "X";
                    }
                    ...
                    if (VL_NEQ_W(words, zerosp, VL_AND_W(words, bufferp, lwp, lwxzp))) {
                        append = "X";
                    } else {
                        append = "Z";
                    }
