...
#define VL_GET_ELEM(suffix, val, idx) \
    ((val)[VL_GET_TYPE_OFFSET(suffix) + ((idx) * VL_GET_TYPE_JUMP(suffix))])
...
static inline WDataOutP VL_EXTENDS_WW(int obits, int lbits, WDataOutP owp,
    ...
    if (VL_SIGN_E_T(lbits, lwp[lwords - 1])) {
...
// clang-format on
VL_UNIOP_CONST_OUT_GEN_HELPER(VL_MOSTSETBITP1_W_GEN)
...
VL_UNIOP_GEN_HELPER(VL_NOT_GEN)
#undef VL_NOT_GEN
...
VL_BIOP_CONST_OUT_GEN_HELPER(VL_EQ_W_GEN)
...
// clang-format on
VL_BIOP_CONST_OUT_GEN_HELPER(_vl_cmp_w_GEN)
#undef _vl_cmp_w_GEN
...
// clang-format on
VL_BIOP_CONST_OUT_GEN_HELPER(_vl_cmps_w_GEN)
...
// clang-format on
VL_UNIOP_GEN_HELPER(VL_NEGATE_W_GEN)
...
static inline WDataOutP VL_MULS_WWW(int lbits, WDataOutP owp, WDataInP const lwp,
    ...
    WDataInP lwusp = lwp;
    WDataInP rwusp = rwp;
    const EData lneg = VL_SIGN_E_T(lbits, lwp[words - 1]);
    if (lneg) {  // Negate lhs
        lwusp = lwstore;
        VL_NEGATE_W_TT(words, lwstore, lwp);
        lwstore[words - 1] &= VL_MASK_E(lbits);  // Clean it
    }
    const EData rneg = VL_SIGN_E_T(lbits, rwp[words - 1]);
    if (rneg) {  // Negate rhs
        rwusp = rwstore;
        VL_NEGATE_W_TT(words, rwstore, rwp);
...
static inline WDataOutP VL_DIVS_WWW(int lbits, WDataOutP owp, WDataInP const lwp,
                                    WDataInP const rwp) VL_MT_SAFE {
    const int lwords = VL_WORDS_I(lbits);
    const EData lsign = VL_SIGN_E_T(lbits, lwp[lwords - 1]);
    const EData rsign = VL_SIGN_E_T(lbits, rwp[lwords - 1]);
    ...
    WDataInP ltup = lwp;
    WDataInP rtup = rwp;
    if (lsign) ltup = _vl_clean_inplace_w(lbits, VL_NEGATE_W_TT(lwords, lwstore, lwp));
    if (rsign) rtup = _vl_clean_inplace_w(lbits, VL_NEGATE_W_TT(lwords, rwstore, rwp));
    if ((lsign && !rsign) || (!lsign && rsign)) {
        WData qNoSign[VL_MULS_MAX_WORDS];
        VL_DIV_WWW_TTT(lbits, qNoSign, ltup, rtup);
        _vl_clean_inplace_w(lbits, VL_NEGATE_W_TT(lwords, owp, qNoSign));
        return owp;
    }
    return VL_DIV_WWW_TTT(lbits, owp, ltup, rtup);
}
static inline WDataOutP VL_MODDIVS_WWW(int lbits, WDataOutP owp, WDataInP const lwp,
    ...
    return VL_MODDIV_WWW_TTT(lbits, owp, ltup, rtup);
}
...
// clang-format on
VL_BIOP_CONST_OUT_GEN_HELPER(_vl_insert_WW_GEN)

// clang-format off
#define _vl_insert_WQ_HELPER(suffix) \
...
// clang-format on
VL_UNIOP_CONST_OUT_GEN_HELPER(_vl_insert_WQ_HELPER)
#undef _vl_insert_WQ_HELPER
...
static inline WDataOutP VL_REPLICATE_WQI(int lbits, WDataOutP owp, QData ld,
    ...
    for (unsigned i = 1; i < rep; ++i) {
        _vl_insert_WQ_T(owp, ld, i * lbits + lbits - 1, i * lbits);
    }
    return owp;
}
static inline WDataOutP VL_REPLICATE_WWI(int lbits, WDataOutP owp, WDataInP const lwp,
    ...
    for (unsigned i = VL_WORDS_I(static_cast<unsigned>(lbits));
        ...
        owp[i] = 0;
    for (unsigned i = 1; i < rep; ++i) {
        _vl_insert_WW_TT(owp, lwp, i * lbits + lbits - 1, i * lbits);
    }
    return owp;
}
...
static inline WDataOutP VL_CONCAT_WWI(int obits, int lbits, int rbits, WDataOutP owp,
    ...
    owp[0] = rd;
    VL_MEMSET_ZERO_W(owp + 1, VL_WORDS_I(obits) - 1);
    _vl_insert_WW_TT(owp, lwp, rbits + lbits - 1, rbits);
    return owp;
}
...
static inline WDataOutP VL_CONCAT_WQI(int obits, int lbits, int rbits, WDataOutP owp, QData ld,
    ...
    owp[0] = rd;
    VL_MEMSET_ZERO_W(owp + 1, VL_WORDS_I(obits) - 1);
    _vl_insert_WQ_T(owp, ld, rbits + lbits - 1, rbits);
    return owp;
}
...
// clang-format on
VL_BIOP_CONST_OUT_GEN_HELPER(VL_SHIFTL_WWI_GEN)
...
static inline WDataOutP VL_SHIFTL_WWQ_##outputSuffix##lhsSuffix##T(int obits, int lbits, int rbits, WDataOutP owp, \
        ...
        VL_SET_WQ(rwp, rd); \
        return VL_SHIFTL_WWW_##outputSuffix##lhsSuffix##T(obits, lbits, rbits, owp, lwp, rwp); \
    }
// clang-format on
...
// clang-format on
VL_UNIOP_CONST_OUT_GEN_HELPER(VL_SHIFTL_IIW_GEN)
...
static inline WDataOutP VL_SHIFTRS_WWW(int obits, int lbits, int rbits, WDataOutP owp,
    ...
    if (VL_UNLIKELY(overshift || rwp[0] >= static_cast<IData>(obits))) {
        ...
        if (VL_SIGN_E_T(lbits, lwp[owords - 1])) {
...
static inline void VL_ASSIGNSEL_WQ(int rbits, int obits, int lsb, WDataOutP iowp,
                                   QData rhs) VL_MT_SAFE {
    _vl_insert_WQ_T(iowp, rhs, lsb + obits - 1, lsb, rbits);
}
static inline void VL_ASSIGNSEL_WW(int rbits, int obits, int lsb, WDataOutP iowp,
                                   WDataInP const rwp) VL_MT_SAFE {
    _vl_insert_WW_TT(iowp, rwp, lsb + obits - 1, lsb, rbits);
}
...
static inline void VL_SELASSIGN_WW(int rbits, int obits, WDataOutP iowp, WDataInP const rwp,
    ...
    _vl_insert_WW_TT(iowp, rwp + wordoff + (lsb != 0), upperbits + obits - 1, upperbits, rbits);
}
