...
bool VlRandomizer::nextFlat(VlRNG& rngr, const std::vector<std::string>& uniqueExprs,
    ...
    // Returns false if it declines, leaving the solve below to run.
...
bool VlRandomizer::Unigen2(VlRNG& rngr, const std::vector<std::string>& uniqueExprs) {
    // relaxSoftConstraints picks the largest compatible subset of softs, which
    // decides which values are legal at all -- leave those to the caller
    // rather than uniformly sampling the wrong solution set.
    if (!m_softConstraints.empty()) return false;

    const size_t currentHash = hashConstraints(uniqueExprs);
    // Params and batch are computed for one constraint set, so drop them
    // once it changes.
    ...
    // twice
    const bool stable = m_ug2PrevHashValid && m_ug2PrevHash == currentHash;
    m_ug2PrevHash = currentHash;
    m_ug2PrevHashValid = true;
    ...
    if (!m_ug2ParamsValid) {
        ...
        if (estimateParameters(solver, rngr, epsilon, hashBits, loTresh, hiTresh)) {
            ...
            m_ug2HasLastSuccess = false;  // new params, so the old hint no longer applies
        } else {
