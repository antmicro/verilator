...
bool VlRandomizer::Unigen2(VlRNG& rngr, const std::vector<std::string>& uniqueExprs) {
    // Soft constraints are not handled in Unigen2
    ...
    if (m_ug2ParamHash != currentHash) {
        ...
        m_ug2IsLargeSpace = false;
        m_ug2ParamHash = currentHash;
        // First sighting of this constraint set. The search below only pays off
        // if its result gets reused, so decline now and let the next call with
        // the same set run it.
        return false;
    }
    ...
    if (!m_ug2ParamsValid) {
        ...
        // caches the result -- the search is the expensive part of the algorithm.
        estimateParameters(solver, rngr);
        m_ug2ParamsValid = true;
        m_ug2LastSuccessI = -1;  // new params, so the old hint no longer applies
    }
    if (generateSamples(solver, rngr) && !m_loTreshWitnesses.empty()) {
        Witness witness = std::move(m_loTreshWitnesses.back());
        m_loTreshWitnesses.pop_back();
        writeBackWitness(witness);
        solver << "(reset)\n";
...
int VlRandomizer::BSAT(std::iostream& solver, int bound, std::vector<Witness>& witnesses,
    ...
    while (static_cast<int>(witnesses.size()) < bound) {
        ...
        if (diversifyRngp && !witnesses.empty()) {
            // Blocking one assignment is too weak on its own: whole batches came back
            // with a constrained variable stuck on one value. Name an element that has
            // to move. Assumed, not asserted, so on unsat the plain query below still
            // runs and the enumerated set is unchanged.
...
// Values used in the large space mode, where the search for a good hash bit count is abandoned and
// a fixed number of XORs is used instead.
...
void VlRandomizer::estimateParameters(std::iostream& solver, VlRNG& rngr) {
    // Below formulas are taken from the UniGen2 paper, Sec. 4, Algorithm 1
    const double pivot = ceil(4.03 * pow((1 + 1 / ug2Kappa), 2));
    m_ug2HiTresh = static_cast<int>(ceil(1 + sqrt(2) * (1 + ug2Kappa) * pivot));
    m_ug2LoTresh = static_cast<int>(floor(pivot / (sqrt(2) * (1 + ug2Kappa))));
    ...
    if (tinyCellSize >= 1 && tinyCellSize <= 60) {
        m_ug2HashBits = 0;
        m_ug2LoTresh = tinyCellSize;  // batch = the whole enumerated set
        m_ug2HiTresh = tinyCellSize + 1;
        return;
    }
    ...
    int solCount = 0;
    // Cap the search with ug2HashBitsLargeSpace: z3 has no native XOR reasoning (the reference
    // UniGen2 uses CryptoMiniSat for that), so each added hash bit roughly doubles solve time.
    // Limit set empirically from measured solve times. Larger spaces cannot be hashed in useful
    // time, so stop and just use fixed number of XORs.
    ...
    m_ug2IsLargeSpace = true;
    m_ug2HashBits = ug2HashBitsLargeSpace;
    m_ug2HiTresh = ug2HiTreshLargeSpace;
}

bool VlRandomizer::generateSamples(std::iostream& solver, VlRNG& rngr) {
    // Try whichever of {hashBits-2, hashBits-1, hashBits} worked last call first, as
    // only the order changes, all three are still tried, so uniformity holds
    // (UniGen2 Sec. 4, "leapfrogging" in spirit but without weakening guarantees).
    // On a large space no cell was ever enumerable, so there is no hint to reuse:
    // go descending, most hash bits first for the smallest cell.
    int candidates[3] = {m_ug2HashBits - 2, m_ug2HashBits - 1, m_ug2HashBits};
    if (m_ug2IsLargeSpace) {
        candidates[0] = m_ug2HashBits;
        candidates[1] = m_ug2HashBits - 1;
        candidates[2] = m_ug2HashBits - 2;
    } else if (m_ug2LastSuccessI >= 0) {
        for (int t = 0; t < 3; ++t) {
    ...
    for (int t = 0; t < 3; ++t) {
        ...
        solver << "(push 1)\n";
        unigenXors(solver, rngr, i);
        // Only on the large space path: there the returned count is not compared
        // against the [loTresh, hiTresh) window, so a query that comes back unsat
        // under its assumption cannot corrupt a cell size decision.
        const int solCount
            = BSAT(solver, m_ug2HiTresh, witnesses, m_ug2IsLargeSpace ? &rngr : nullptr);
        if (m_ug2LoTresh <= solCount && (solCount < m_ug2HiTresh || m_ug2IsLargeSpace)) {
            m_ug2LastSuccessI = i;
            const int take
                = m_ug2IsLargeSpace ? std::min(ug2BatchLargeSpace, solCount) : m_ug2LoTresh;
            ...
            for (int k = 0; k < take; ++k) {
                ...
                m_loTreshWitnesses.push_back(std::move(witnesses[indices[k]]));
            }
            solver << "(pop 1)\n";
            return true;
        }
