...
static AstNodeExpr* sampled(AstNodeExpr* exprp) {
    ...
    sp->dtypeFrom(exprp);
    if (exprp->width() == 1) return sp;
    return new AstRedOr{exprp->fileline(), sp};
}
...
public:
    ...
    AstNodeExpr* lower(FileLine* flp, SvaGraph& graph, AstNodeExpr* triggerExprp,
        ...
        const SignalSet sigs = computeSignals(c, outRequiredStepSrcsp, outPassMatchSrcsp);
...
class AssertNfaVisitor final : public VNVisitor {
    ...
    void attachMatchHandlers(FileLine* flp, AstAssert* assertAssertp, AstAssert* assertWithFailp,
        ...
        if (requiredStepSrcsp) {
            ...
            for (AstNodeExpr* const srcp : *requiredStepSrcsp) pushDeletep(srcp);
        }
        ...
        if (passMatchSrcsp) {
            ...
            VL_DO_DANGLING(pushDeletep(cumulativeOrp), cumulativeOrp);
            for (AstNodeExpr* const srcp : *passMatchSrcsp) pushDeletep(srcp);
        }
