...
private:
    ...
    AstVar* m_insidePexprEvtVarp
        = nullptr;  // Event fired at begining of pexpr (observed region). Used for delay control
                    // (waiting for senip directly would end up in wrong region)
    // Other:
    ...
    V3UniqueNames m_insidePexprEvtNames{
        "__VinsidePexprEvt"};  // Inside PExpr event var name generator
    ...
    void visit(AstDelay* nodep) override {
        ...
        if (!m_defaultClockingp) {
            ...
            sensesp = new AstSenItem{flp, VEdgeType::ET_EVENT,
                                     new AstVarRef{flp, m_insidePexprEvtVarp, VAccess::READ}};
        } else {
    ...
    void visit(AstPExpr* nodep) override {
        ...
        AstFireEvent* fireEv = new AstFireEvent{
            flp, new AstVarRef{flp, m_insidePexprEvtVarp, VAccess::WRITE}, false};
