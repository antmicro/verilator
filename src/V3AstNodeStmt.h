...
public:
    ...
    bool sameNode(const AstNode* samep) const override {
        ...
        // As V3Combine is running after trace updates are set up, we are safe to do string-based
        // deduplication ignoring actual `valuep` that are underheath
        return dtypep()->isSame(otherp->dtypep()) && m_code == otherp->m_code
