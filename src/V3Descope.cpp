...
private:
    ...
    struct ScopeSelfPtr final {
        VSelfPointer thisPtr = VSelfPointer::makeNone();
        VSelfPointer vlSymsPtr = VSelfPointer::makeNone();
    };
    ...
    VSelfPointer scopeThis(const AstScope* scopep) {
        auto& ret = m_scopeToSelf[scopep];
        if (ret.thisPtr.isNone()) {
            string name = scopep->name();
            string::size_type pos;
            if ((pos = name.rfind('.')) != string::npos) name.erase(0, pos + 1);
            ret.thisPtr = VSelfPointer::makeThis(name);
        }
        return ret.thisPtr;
    }
    // Construct a 'vlSyms' self pointer for the given scope
    VSelfPointer scopeVlSyms(const AstScope* scopep) {
        auto& ret = m_scopeToSelf[scopep];
        if (ret.vlSymsPtr.isNone()) {
            ret.vlSymsPtr = VSelfPointer::makeVlSyms(scopep->nameDotless());
        }
    ...
    // module.
    VSelfPointer descopedSelfPointer(const AstScope* scopep) {
    ...
    void visit(AstNodeVarRef* nodep) override {
        ...
        if (varp->isFuncLocal()) {
            ...
            nodep->selfPointer(VSelfPointer::makeNone());
