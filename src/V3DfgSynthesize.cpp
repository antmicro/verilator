...
class AstToDfgConverter final : public VNVisitor {
    ...
    void visit(AstArraySel* nodep) override {
        ...
        iterate(nodep->fromp());
        if (m_foundUnhandled) return;
        UASSERT_OBJ(nodep->fromp()->user2p(), nodep, "fromp missing Dfg vertex");
        iterate(nodep->bitp());
        if (m_foundUnhandled) return;
        UASSERT_OBJ(nodep->bitp()->user2p(), nodep, "bitp missing Dfg vertex");
        ...
        vtxp->fromp(nodep->fromp()->user2u().to<DfgVertex*>());
        vtxp->bitp(nodep->bitp()->user2u().to<DfgVertex*>());
