...
class EmitCImp final : EmitCFunc {
    ...
    void emitCommonImp(const AstNodeModule* modp) VL_EXCLUDES(v3Global.filesMutex()) {
...
void V3EmitC::emitcImp() VL_EXCLUDES(v3Global.constPoolMutex(), v3Global.typeTableMutex()) {
...

    {
        ...
        for (const AstNode* nodep = v3Global.netlistcp()->modulesp(); nodep;
             nodep = nodep->nextp()) {
            if (VN_IS(nodep, Class)) continue;  // Imped with ClassPackage
            const AstNodeModule* const modp = VN_AS(nodep, NodeModule);
            cfiles.emplace_back();
            auto& slowCfilesr = cfiles.back();
            futures.push_back(V3ThreadPool::s().enqueue([modp, &slowCfilesr]() {
                VL_SHARED_LOCK_GUARD(constPoolMutexLockGuard, v3Global.constPoolMutex());
                VL_SHARED_LOCK_GUARD(typeTableMutexLockGuard, v3Global.typeTableMutex());
                EmitCImp::main(modp, /* slow: */ true, slowCfilesr);
            }));
            cfiles.emplace_back();
            auto& fastCfilesr = cfiles.back();
            futures.push_back(V3ThreadPool::s().enqueue([modp, &fastCfilesr]() {
                VL_SHARED_LOCK_GUARD(constPoolMutexLockGuard, v3Global.constPoolMutex());
                VL_SHARED_LOCK_GUARD(typeTableMutexLockGuard, v3Global.typeTableMutex());
                EmitCImp::main(modp, /* slow: */ false, fastCfilesr);
            }));
        }
        ...
        if (v3Global.opt.trace() && !v3Global.opt.lintOnly()) {
            ...
            futures.push_back(V3ThreadPool::s().enqueue([&slowCfilesr]() {
                VL_SHARED_LOCK_GUARD(constPoolMutexLockGuard, v3Global.constPoolMutex());
                VL_SHARED_LOCK_GUARD(typeTableMutexLockGuard, v3Global.typeTableMutex());
                EmitCTrace::main(v3Global.netlistcp()->topModulep(), /* slow: */ true,
                                 slowCfilesr);
            }));
            cfiles.emplace_back();
            auto& fastCfilesr = cfiles.back();
            futures.push_back(V3ThreadPool::s().enqueue([&fastCfilesr]() {
                VL_SHARED_LOCK_GUARD(constPoolMutexLockGuard, v3Global.constPoolMutex());
                VL_SHARED_LOCK_GUARD(typeTableMutexLockGuard, v3Global.typeTableMutex());
                EmitCTrace::main(v3Global.netlistcp()->topModulep(), /* slow: */ false,
                                 fastCfilesr);
            }));
        }
        // Wait for futures
