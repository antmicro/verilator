...
class EmitCImp final : EmitCFunc {
    ...
    std::shared_ptr<std::deque<AstCFile*>> m_filenames;  // filenames generated by this emit
    ...
    void openNextOutputFile(const std::set<string>& headers, const string& subFileName) {
        ...
        if (v3Global.opt.lintOnly()) {
            ...
            m_filenames->push_back(
                newCFile(filename, /* slow: */ m_slow, /* source: */ true, false));
    ...
    explicit EmitCImp(const AstNodeModule* modp, bool slow,
                      std::shared_ptr<std::deque<AstCFile*>> filenames)
        : m_fileModp{modp}
        , m_slow{slow}
...
class EmitCTrace final : EmitCFunc {
    ...
    void openNextOutputFile() {
        ...
        AstCFile* const cfilep = newCFile(filename, m_slow, true /*source*/, false);
        cfilep->support(true);
        m_filenames->push_back(cfilep);
...
public:
    static void main(AstNodeModule* modp, bool slow,
                     std::shared_ptr<std::deque<AstCFile*>> filenames) {
        EmitCTrace{modp, slow, filenames};
    }
};
...
void V3EmitC::emitcImp() {
    ...
    std::deque<std::shared_ptr<std::deque<AstCFile*>>> m_filenames;

    // Process each module in turn
    for (const AstNode* nodep = v3Global.rootp()->modulesp(); nodep; nodep = nodep->nextp()) {
        if (VN_IS(nodep, Class)) continue;  // Imped with ClassPackage
        const AstNodeModule* const modp = VN_AS(nodep, NodeModule);
        auto slow = std::make_shared<std::deque<AstCFile*>>();
        auto fast = std::make_shared<std::deque<AstCFile*>>();
        m_filenames.push_back(slow);
        m_filenames.push_back(fast);
        m_futures.push_back(v3ThreadPool.enqueue<void>(
    ...
    if (v3Global.opt.trace() && !v3Global.opt.lintOnly()) {
        ...
        m_filenames.push_back(slow);
        m_filenames.push_back(fast);
        m_futures.push_back(v3ThreadPool.enqueue<void>([slow]() {
            EmitCTrace::main(v3Global.rootp()->topModulep(), /* slow: */ true, slow);
        }));
        m_futures.push_back(v3ThreadPool.enqueue<void>([fast]() {
