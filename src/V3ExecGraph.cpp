...
private:
    ...
    // Debugging
    static size_t longestModuleName(const std::vector<ThreadSchedule>& schedules) {
        size_t longestModuleName = 0;
        for (const ThreadSchedule& schedule : schedules) {
            for (const auto& thread : schedule.threads) {
                for (const ExecMTask* const mtaskp : thread) {
                    longestModuleName = std::max(longestModuleName, mtaskp->modName().length());
                }
            }
        }
        return longestModuleName;
    }
    static void dumpDotFile(const std::vector<ThreadSchedule>& schedules, const V3Graph& graph,
        ...
        *logp << "\n  // Threads\n";
        ...
        const string hierBlockLabel = "\\nhier_block='";
        constexpr double minWidth = 2.0;
        const double threadBoxWidth = std::max(
            static_cast<double>(longestModuleName(schedules) + hierBlockLabel.length()) * 0.1,
            minWidth);
        constexpr double threadBoxHeight = 1.5;
        const double horizontalGap = threadBoxWidth / 2;
        for (int i = 0; i < v3Global.opt.threads(); i++) {
            const string name = "t" + std::to_string(i);
            const string label = "Thread " + std::to_string(i);
            const double posX = -threadBoxWidth / 2;
        ...
        *logp << "\n  // MTasks\n";
        ...
        const auto emitMTask = [&](const ExecMTask* const mtaskp, int index,
            ...
            for (int i = 0; i < mtaskp->threads(); ++i) {
                ...
                const string label
                    = mtaskp->name() + " (" + std::to_string(startTime(mtaskp)) + ':'
                      + std::to_string(endTime(mtaskp)) + ')'
                      + (mtaskp->modName().empty() ? ""
                                                   : hierBlockLabel + mtaskp->modName() + '\'')
        ...
        const auto emitFork = [&](int index) {
            ...
            const double width = threadBoxWidth / 8;
