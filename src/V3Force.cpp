...
public:
    ...
    struct ForceInfo final {
        ...
        bool m_isConstGroup;  // True if using shared constant value storage

        ForceInfo(AstNodeExpr* rhsExprp, AstVarScope* enVscp, bool isConstGroup = false)
            : m_rhsExprp{rhsExprp}
            , m_enVscp{enVscp}
            , m_isConstGroup{isConstGroup} {}
    };
    ...
    // STATIC METHODS
    // Replace each AstNodeVarRef in the given 'nodep' that writes a variable by
    // transforming the referenced AstVarScope with the given function.
    ...
    AstNodeExpr* createForceReadExpression(
        ...
        AstVarRef* originalRefp, AstNodeExpr* lhsp) const {
        ...
        for (const auto& force : forces) {
            ...
            AstNodeExpr* rhsClonep
                = (force.second.m_isConstGroup)
                      ? new AstVarRef{originalRefp->fileline(),
                                      m_constValByVar.find(originalRefp->varp())->second,
                                      VAccess::READ}
                      : force.second.m_rhsExprp->cloneTreePure(false);

            // Create: __VforceEnID & RHSID (or conditional for non-ranged)
            if (ForceState::isRangedDType(lhsp)) {
                // This happens when an earlier stage transforms a structure into an array
                if (lhsForceEnp->width() != rhsClonep->width()) {
                    if (lhsForceEnp->width() > rhsClonep->width()) {
                        // Zero-extend to match the width
                        rhsClonep = new AstExtend{originalRefp->fileline(), rhsClonep,
                                                  lhsForceEnp->width()};
                    } else {
                        // Truncate to match the width
                        rhsClonep = new AstSel{originalRefp->fileline(), rhsClonep, 0,
                                               lhsForceEnp->width()};
                    }
                }
...
private:
    ...
    // Shared enable/value per variable for grouped-constant forces
    std::unordered_map<AstVar*, AstVarScope*> m_constEnByVar, m_constValByVar;
...
public:
    ...
    void addForceAssignment(AstVar* varp, AstVarScope* vscp, AstNodeExpr* rhsExprp,
        ...
        const bool isConst
            = !rhsExprp->exists([](const AstVarRef* /*refp*/) -> bool { return true; });
        ...
        auto newEnable = [&]() {
            auto* ev = new AstVar{
                ...
                (ForceState::isRangedDType(varp) ? varp->dtypep() : varp->findBitDType())};
            varp->addNextHere(ev);
            auto* v = new AstVarScope{varp->fileline(), scopep, ev};
            scopep->addVarsp(v);
            return v;
        };

        auto initEnableZero = [&](AstVarScope* v) {
            V3Number z{v, v->width()};
            z.setAllBits0();
            auto* a
                = new AstAssign{flp, new AstVarRef{flp, v, VAccess::WRITE}, new AstConst{flp, z}};
            auto* act = new AstActive{
                flp, "force-init", new AstSenTree{flp, new AstSenItem{flp, AstSenItem::Static{}}}};
            act->senTreeStorep(act->sentreep());
            act->addStmtsp(new AstInitial{flp, a});
            scopep->addBlocksp(act);
        };
        AstVarScope* enVscp = nullptr;
        auto it = isConst ? m_constEnByVar.find(varp) : m_constEnByVar.end();
        if (isConst && it != m_constEnByVar.end()) {
            enVscp = it->second;
        } else {
            enVscp = newEnable();
            initEnableZero(enVscp);
            if (isConst) {
                auto* vv = new AstVar{varp->fileline(), VVarType::VAR,
                                      varp->name() + "__VforceValC", varp->dtypep()};
                varp->addNextHere(vv);
                auto* vvs = new AstVarScope{varp->fileline(), scopep, vv};
                scopep->addVarsp(vvs);
                m_constEnByVar.emplace(varp, enVscp);
                m_constValByVar.emplace(varp, vvs);
            }
        }
        m_forceInfo[varp].emplace(forceStmtp, ForceInfo{rhsExprp, enVscp, isConst});
    }
    ...
    AstVarScope* getConstValVscp(AstVar* varp) const {
        auto it = m_constValByVar.find(varp);
        return it == m_constValByVar.end() ? nullptr : it->second;
    }
};
...
class ForceConvertVisitor final : public VNVisitor {
    ...
    void visit(AstAssignForce* nodep) override {
        ...
        AstVarScope* const currentEnVscp = currentForceInfo.m_enVscp;

        // If this force is part of a grouped-constant set, assign the shared value first
        AstNode* stmtListp = nullptr;
        if (currentForceInfo.m_isConstGroup) {
            AstNodeExpr* valLhsp
                = new AstVarRef{flp, m_state.getConstValVscp(forcedVarp), VAccess::WRITE};
            AstNodeExpr* valRhsp = currentForceInfo.m_rhsExprp->cloneTreePure(false);
            if (ForceState::isRangedDType(lhsp)) {
                if (valLhsp->width() != valRhsp->width()) {
                    if (valLhsp->width() > valRhsp->width()) {
                        valRhsp = new AstExtend{nodep->fileline(), valRhsp, valLhsp->width()};
                    } else {
                        valRhsp = new AstSel{nodep->fileline(), valRhsp, 0, valLhsp->width()};
                    }
                }
            }
            AstAssign* const assignValp = new AstAssign{flp, valLhsp, valRhsp};
            stmtListp = assignValp;
        }
        ...
        UINFO(3, "Replaced force with enable logic: " << currentForceInfo.m_enVscp->name());
