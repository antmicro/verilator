...
bool VString::wildmatchi(const char* s, const char* p) VL_MT_SAFE {
    for (; *p; s++, p++) {
        if (*p != '*') {
...
bool VString::wildmatch(const char* s, const char* p) VL_MT_SAFE {
    for (; *p; s++, p++) {
        if (*p != '*') {
            if (((*s) != (*p)) && *p != '?') return false;
        } else {
...
bool VString::wildmatch(const string& s, const string& p) VL_MT_SAFE {
...
string VString::spaceUnprintable(const string& str) VL_MT_SAFE {
    string out;
    for (const char c : str) {
        if (isprint(c)) {
            out += c;
...
VL_ATTR_ALWINLINE
static uint32_t shaRotr32(uint32_t lhs, uint32_t rhs) VL_MT_SAFE {
    return lhs >> rhs | lhs << (32 - rhs);
}

VL_ATTR_ALWINLINE
static void sha256Block(uint32_t* h, const uint32_t* chunk) VL_MT_SAFE {
    uint32_t ah[8];
