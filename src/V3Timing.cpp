...
class TimingControlVisitor final : public VNVisitor {
    ...
    static bool staticallyNonZeroDelay(const AstNodeExpr* valuep) {
        ...
        if (const AstRToIRoundS* const roundp = VN_CAST(valuep, RToIRoundS)) {
            return staticallyNonZeroDelay(roundp->lhsp());
        }
        if (const AstISToRD* const istordp = VN_CAST(valuep, ISToRD)) {
            return staticallyNonZeroDelay(istordp->lhsp());
        }
        ...
        if (const AstMulD* const mulp = VN_CAST(valuep, MulD)) {
            const AstConst* const lhsConstp = VN_CAST(mulp->lhsp(), Const);
            const AstConst* const rhsConstp = VN_CAST(mulp->rhsp(), Const);
            if (lhsConstp) return staticallyNonZeroDelay(mulp->rhsp());
            if (rhsConstp) return staticallyNonZeroDelay(mulp->lhsp());
        }
        return false;
    }
    ...
    void visit(AstAssignW* nodep) override {
        ...
        if (AstDelay* const delayp = VN_CAST(controlp, Delay)) {
            if (AstNodeExpr* const fallDelayp
                = delayp->fallDelayp() ? delayp->fallDelayp()->unlinkFrBack() : nullptr) {
