...
class UdpVisitor final : public VNVisitor {
    ...
    std::map<AstVar*, std::pair<AstAlways*, AstAlways*>>
        m_trigAlwaysMapp;  // The always block map for r|f edge.
    ...
    void addEdgTriglogic(AstUdpTableLine* nodep, std::vector<std::string>& iFieldNames,
        ...
        if (trigStr == "r") {
            if (m_trigAlwaysMapp[m_trigVarp].first == nullptr) {
                ...
                m_trigAlwaysMapp[m_trigVarp].first = new AstAlways{
            ...
            if (m_trigAlwaysMapp[m_trigVarp].second == nullptr) {
                AstVarRef* varRefp = new AstVarRef{fl, m_trigVarp, VAccess::READ};
                m_trigAlwaysMapp[m_trigVarp].second = new AstAlways{
                    fl, VAlwaysKwd::ALWAYS,
