    ...
    void visit(AstAlias* nodep) override {
        ...
        for (AstNode* itemp = nodep->itemsp()->nextp(); itemp; itemp = itemp->nextp()) {
            ...
            if (!firstItemDtypep->similarDType(itemp->dtypep())) {
                itemp->v3error("Incompatible data types of nets used for net alias. First operand has the type " << firstItemDtypep->prettyDTypeNameQ() << ", other has " << itemp->dtypep()->prettyDTypeNameQ());
            }
        }
    }
