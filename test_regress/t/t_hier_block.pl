...
file_grep($Self->{obj_dir} . "/Vsub0_/sub0_.sv", qr/^\s+\/\/\s+timeprecision\s+(\d+)ps;/mi, 1);
file_grep($Self->{obj_dir} . "/Vsub0_/sub0_.sv", /^module\s+(\S+)\s+/, "sub0");
file_grep($Self->{obj_dir} . "/Vsub1_/sub1_.sv", /^module\s+(\S+)\s+/, "sub1");
file_grep($Self->{obj_dir} . "/Vsub2_/sub2_.sv", /^module\s+(\S+)\s+/, "sub2");
