...
module t;
  ...
  initial begin
    ...
    for (int i = 0; i < NUM_ITERS; ++i) begin
      ...
      if ($countones(c.x) != 4) begin
        $display("%%Error: constraint violated, x=%0d has %0d bits set", c.x, $countones(c.x));
        $stop;
      end
      if (!seen[c.x]) begin
        seen[c.x] = 1'b1;
    ...
    if (distinct != NUM_SOLUTIONS) begin
      $display("%%Error: only %0d/%0d distinct values covered", distinct, NUM_SOLUTIONS);
      $stop;
    end
    $write("*-* All Finished *-*\n");
    $finish;
